...
function initializeTitleBar(isShareNeeded, isRtl) {
    if (!isRtl) {
        ...
        downloadTooltip = 'Download the document';
        saveToolTip = 'Save document in server';
        printText = 'Print';
        ...
        saveText = 'يحفظ';
        saveToolTip = 'احفظ المستند في الخادم';
        downloadText ='تحميل';
        downloadTooltip ='قم بتنزيل المستند';
        printText = 'طباعه';
        printToolTip = 'طباعه هذا المستند (Ctrl + P)';
        openText = 'فتح';
        documentTileText = 'اسم المستند. انقر أو اضغط لأعاده تسميه هذا المستند';
    }
    documentTitle = ej.base.createElement('label', { id: 'documenteditor_title_name', styles: 'font-weight:400;text-overflow:ellipsis;white-space:pre;overflow:hidden;user-select:none;cursor:text;float:left;padding-left:10px' });
    ...
    var btnStyles = btnFloatStyle + 'background: transparent;box-shadow:none; font-family: inherit;border-color: transparent;' + 'border-radius: 2px;color:inherit;font-size:12px;text-transform:capitalize;margin-top:4px;height:28px;font-weight:400';
    printBtn = addButton('e-de-icon-Print ' + iconCss, printText, btnStyles, 'de-print', printToolTip, false);
    ...
    saveBtn = addButton('e-icons e-save ' + iconCss, saveText, btnStyles, 'de-save', saveToolTip, false);
    if (!isShareNeeded) {
...
function wireEventsInTitleBar() {
    ...
    saveBtn.element.addEventListener('click', onSave);
...
function onSave() {
    ...
    http.open('POST', window.location.origin + '/api/documenteditor/ExportSFDT');
    ...
    http.send(JSON.stringify({ Content: container.documentEditor.serialize(), FileName: fileName}));
}
...
function onPrint() {
    var documentContainer = document.getElementById("DocumentEditor").ej2_instances[0];
...
function save(format) {
    var editor = document.getElementById("DocumentEditor").ej2_instances[0].documentEditor;
